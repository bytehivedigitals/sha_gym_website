...
<div className="landing-container">
  <div className={`relative bg-black text-white flex ${isMobile ? 'w-[100%] min-h-[fit-content] flex-col' : 'min-h-[100vh] justify-center'} overflow-hidden`}>
    <div className="relative w-full">
      {/* Text in the background - make sure it has higher z-index than image */}
      <div className={`landing-header relative z-20 flex ${isMobile ? 'text-left text-2xl ml-4' : 'justify-center text-center px-4 mt-6'}`}>
      ...
      {/* Image in the foreground - lower z-index than text */}
      <img
        src={isMobile ? hitmanMobile : hitman}
        alt="Hitman"
        className={`${isMobile ? 'absolute right-0 object-cover z-0 translate-y-[-70%]' : 'absolute inset-0 w-full h-full object-contain z-10 translate-y-[-8%]'}`}
      />
    </div>
